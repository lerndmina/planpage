...
### Interactive widgets (JS mode)

**Never write your own checkbox/persistence/tab JavaScript** — the chrome ships a widget runtime that is injected automatically whenever the fragment uses the markup below. All state persists in the reader's browser (localStorage), survives republishing (items are keyed by index + leading text), and pages with stateful widgets get a sticky strip with a progress bar plus **Copy state** / **Reset** buttons. Copy state serializes every checkbox, step status, decision, and note on the page into one markdown blob the reader pastes back to an agent — expect users to hand you that blob, and treat it as the current status of the plan (the header line carries the page URL).

- **Checklist**: `<ul class="check"><li>Task text</li></ul>` — plain `li`s become persistent checkboxes with strike-through; `<li data-checked>` pre-checks an item (work already done at publish time). Give the `ul` an `id` and put `<span class="count" data-count="that-id"></span>` in a heading for a live `2/5` chip. In markdown mode, GFM `- [ ]` / `- [x]` items convert automatically.
- **Step tracker**: `<ol class="steps track">` — the standard step list, but clicking a step's number badge cycles pending → doing → done → blocked (persisted, feeds the progress bar). Keep the `<li><strong>Title</strong><br>detail…` shape; the `<strong>` is what Copy state uses as the step name.
- **Decision picker**: `<table class="decide" data-name="Database">` where each `tbody` row is one competing option (first cell = option name). Readers click a row to pick it (click again to unpick); the pick persists and shows in Copy state as `- Database: <option>`. Without `data-name` the nearest heading names the decision. One decision per table.
- **Notes**: drop a single `<div class="notes"></div>` (optionally `data-name="…"`) where general feedback belongs — it renders a persistent textarea, and its presence also adds an "✎ note" affordance to every `h2` so readers can annotate per section. Notes come back to you via Copy state quoted under their section headings.
- **Tabs**: `<div class="tabs"><section title="macOS">…</section><section title="Linux">…</section></div>` — last active tab is remembered. Degrades to stacked titled sections without JS.
